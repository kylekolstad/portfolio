...
function FormattedOutputText({ text }: { text: string }) {
  const keyValueMatch = text.match(/^(\s*)([A-Za-z][\w -]*)(:)(.*)$/);

  if (keyValueMatch) {
    const [, indent, key, colon, value] = keyValueMatch;
    const normalizedKey = key.trim().toLowerCase();
    const isFocus = normalizedKey === "focus";

    return (
      <>
        {indent}
        <span className="text-foreground dark:text-neutral-300">{key}</span>
        <span className="text-muted-foreground dark:text-neutral-500">
          {colon}
        </span>
        {isFocus ? (
          <StackLikeOutputText text={value} />
        ) : (
  ...
  return (
    <>
      {jsonParts.map((part, i) => {
        ...
        if (/^"(?:[^"\\]|\\.)*"$/.test(part)) {
          const nextToken = jsonParts
            .slice(i + 1)
            .find((candidate) => candidate.trim());
          const isKey = nextToken === ":";
          ...
          return (
            <span
              ...
              className={
                isKey
                  ? "text-[var(--accent-blue)] dark:text-[#9CDCFE]"
        ...
        if (/^[{}\[\],:]$/.test(part)) {
          return (
            <span key={i} className="text-foreground dark:text-neutral-300">
              {part}
            </span>
          );
        }
        ...
        return (
          <span key={i} className="text-muted-foreground dark:text-neutral-400">
            {part}
          </span>
        );
      })}
...
function StackLikeOutputText({ text }: { text: string }) {
  ...
  return (
    <>
      {parts.map((part, i) => {
        if (/^,\s*$/.test(part)) {
          return (
            <span key={i} className="text-muted-foreground dark:text-neutral-500">
              {part}
            </span>
          );
        }
          ...
          {lines.map((line, i) => (
            ...
            >
              {line.type === "command" ? (
                ...
                <span className="text-muted-foreground dark:text-neutral-400">
                  <FormattedOutputText text={line.content} />
                </span>
              )}
            </div>
          ))}
